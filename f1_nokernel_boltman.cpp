...
void diff_nonlinear(double *p1_n, double *q1_n, double *rho_n) {
  ...
  double coeff = 1.0 / (2 * alpha0 * sig0 * rho_0 + lambda0 - lambda0 * pk21);
  double Betaa = 2.0 * alpha0 * D_n * pk22 * coeff;                   
  double xii = 16 * alpha0 * alpha0 * pk12 * pk22 * coeff / (3 * PI);
  double gammaa = 16 * gamma0 * alpha0 * D_n * pk12 * coeff / (3 * PI);
  double dd0 = 0.5;

  for (i = 0; i < YN; i++) {
    muu[i] = (4 * pk12 / PI - 1) * 2 * alpha0 * rho[i] - lambda0 * (1 - pk11);
    deltap_q[i] =
        (p1[i] * p1[i] -
         q1[i] * q1[i]);  
    deltapq[i] = (p1[i] * q1[i]);
  ...
  for (i = 0; i < YN; i++) {
    ...
    p1_n[i] -= 0.25 * Betaa * (deltap_q[Ai] + deltap_q[Bi] - 2 * deltap_q[i]) * dyy;  // Beta term. 
    p1_n[i] += 0.125 * gammaa * q1[i] * (q1[Ai] + q1[Bi] - 2 * q1[i]) * dyy;  // Gamma term.
  ...
  for (i = 0; i < YN; i++) {
    ...
    q1_n[i] = -0.50 * Betaa * (deltapq[Ai] + deltapq[Bi] - 2 * deltapq[i]) * dyy;
...
void implicit_method(double *p1_n, double *q1_n, double *rho_n) {
  ...
  int i;
  double gammaa =
      16 * gamma0 * alpha0 * D_n * pk12 /
      (3 * PI * (2 * alpha0 * sig0 * rho_0 + lambda0 - lambda0 * pk21));
  /*
   * f_1^R Equation.
   */
  for (i = 0; i < YN; i++) {
    Di[i + 1] = 0.5 * D_p + 0.125 * gammaa * p1[i];  // Gamma terms introduce diffusion. D_p = 1.
  }
...
//  rho_0 = PI * lambda0 * (1 - pk11) / (2 * alpha0 * (4 * pk12 - PI));
  pk11 = PI * (lambda0 + 2 * alpha0 * rho_0) / (8 * alpha0 * rho_0 + PI * lambda0); // Fix rho_0 = 1.
  pk12 = pk11;
  // Change pk22 and alpha0.
  pk21 = 1.0;
  ...
  printf(
      "alpha0, lambda0, pk11, pk21, pk12, pk22, D_0, rho_0 = %f,%f,%f,%f,%f,%f,%f,%f\n",
